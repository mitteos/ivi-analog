...
export const MedallionsItem: React.FC<MedallionsItemProps> = ({img, title}) => {
  return (
      <Link href={img ? "/actor/1" : "/"} className='flex flex-col gap-y-2 group cursor-pointer w-[85px]'>
        ...
        <p className='text-gray-600 group-hover:text-gray-300 text-xs transition-colors duration-300 text-center'>{title}</p>
      </Link>
  );
};
